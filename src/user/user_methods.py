...
class UserMethods:
    ...
    def add_new_user(user: User) -> None:
        try:
            ...
            conn.insert(DatabaseSchema.YOUTUBE_DOWNLOADER, DatabaseTables.USERS, user_dict)
            conn.commit()
    ...
    def get_user(email: str) -> Dict:
        try:
            conn = get_db()
            user = conn.get_one(DatabaseSchema.YOUTUBE_DOWNLOADER,
                                DatabaseTables.USERS, '*', ("email_id = %s", [email]))
            return user
        except Exception as e:
    ...
    def get_user_by_username(username: str) -> Dict:
        try:
            ...
            user = conn.get_one(DatabaseSchema.YOUTUBE_DOWNLOADER,
                                DatabaseTables.USERS, '*', ("user_name = %s", [username]))
            return user
        except Exception as e:
    ...
    def update_user_data(user_identifier: str,
        ...
        conn = get_db()
        user = conn.update(DatabaseSchema.YOUTUBE_DOWNLOADER, DatabaseTables.USERS,
                           fields_to_update, (by+"=%s", (user_identifier,)))
        if user:
            conn.commit()
            return user
        else:
